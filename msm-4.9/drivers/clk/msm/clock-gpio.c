...
{
    GCC_MM_GP_CMD_RCGR = 0x54000 + (gpio_nr - 36) * 0x1000;
    GCC_MM_GP_CFG_RCGR = 0x54004 + (gpio_nr - 36) * 0x1000;
    GCC_MM_GP_M        = 0x54008 + (gpio_nr - 36) * 0x1000;
    GCC_MM_GP_N        = 0x5400C + (gpio_nr - 36) * 0x1000;
    GCC_MM_GP_D        = 0x54010 + (gpio_nr - 36) * 0x1000;
    GCC_CAMSS_GP_CBCR  = 0x54018 + (gpio_nr - 36) * 0x1000;
}
...
static void close_pwm_base (void)
{
   // int i;
    gpio_config(33, 0, 1, 0, 0);
    writel_relaxed(0, base_gpio + 0x00000004 + 33 * 0x1000);
}
...
static int gpio_clk_probe (struct platform_device *pdev)
{
    ...
    if (gpio_nr != 33 && gpio_nr != 36) {
        pr_err("---+++ pwm-gpio must be 33 or 36\n");
        return -EINVAL;
    }
