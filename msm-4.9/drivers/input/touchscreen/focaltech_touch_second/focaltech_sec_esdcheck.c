...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_sec_read_reg(FTS_REG_ESD_SATURATE, &val);
    if ( ret < 0) {
...
static int fts_sec_esdcheck_tp_reset(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
...
static bool get_chip_id(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        ret = fts_sec_read(&reg_addr, 1, &reg_value, 1);
        if (ret < 0) {
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_sec_read_reg(reg_addr, &reg_value);
    if ( ret < 0 ) {
        fts_esdcheck_data.nack_cnt++;
    ...
    if ( hardware_reset == 1) {
        FTS_DEBUG("[ESD]: NoACK=%d, Error Data=%d, Hardware Reset=%d", \
                  ...
                  fts_esdcheck_data.hardware_reset_cnt);
        fts_sec_esdcheck_tp_reset(ts_data);
    }
...
int fts_sec_esdcheck_get_status(void)
{
    /* interrupt don't add debug message */
...
/*****************************************************************************
*  Name: fts_sec_esdcheck_proc_busy
...
static ssize_t fts_sec_esdcheck_store(
    struct device *dev,
...
static ssize_t fts_sec_esdcheck_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    count = snprintf(buf, PAGE_SIZE, "Esd check: %s\n", \
                     fts_sec_esdcheck_get_status() ? "On" : "Off");
    mutex_unlock(&input_dev->mutex);
 ...
 *
 */
static DEVICE_ATTR (fts_esd_mode, S_IRUGO | S_IWUSR, fts_sec_esdcheck_show, fts_sec_esdcheck_store);
...
int fts_sec_create_esd_sysfs(struct device *dev)
{
    int ret = 0;
...
int fts_sec_esdcheck_init(struct fts_ts_data *ts_data)
{
    ...
    fts_esdcheck_data.mode = ENABLE;
    fts_sec_esdcheck_switch(ENABLE);
    fts_sec_create_esd_sysfs(ts_data->dev);
    FTS_FUNC_EXIT();
    return 0;
}
