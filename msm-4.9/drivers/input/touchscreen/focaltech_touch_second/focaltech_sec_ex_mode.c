...
*******************************************************************************/
static int fts_sec_ex_mode_switch(enum _ex_mode mode, u8 value)
{
    int ret = 0;
    u8 m_val = 0;
    ...
    switch (mode) {
    case MODE_GLOVE:
        ret = fts_sec_write_reg(FTS_REG_GLOVE_MODE_EN, m_val);
        if (ret < 0) {
            FTS_ERROR("MODE_GLOVE switch to %d fail", m_val);
        }
        break;
    case MODE_COVER:
        ret = fts_sec_write_reg(FTS_REG_COVER_MODE_EN, m_val);
        if (ret < 0) {
            FTS_ERROR("MODE_COVER switch to %d fail", m_val);
        }
        break;
    case MODE_CHARGER:
        ret = fts_sec_write_reg(FTS_REG_CHARGER_MODE_EN, m_val);
        if (ret < 0) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_sec_read_reg(FTS_REG_GLOVE_MODE_EN, &val);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!ts_data->glove_mode) {
            ...
            ret = fts_sec_ex_mode_switch(MODE_GLOVE, ENABLE);
            if (ret >= 0) {
        ...
        if (ts_data->glove_mode) {
            ...
            ret = fts_sec_ex_mode_switch(MODE_GLOVE, DISABLE);
            if (ret >= 0) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_sec_read_reg(FTS_REG_COVER_MODE_EN, &val);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!ts_data->cover_mode) {
            ...
            ret = fts_sec_ex_mode_switch(MODE_COVER, ENABLE);
            if (ret >= 0) {
        ...
        if (ts_data->cover_mode) {
            ...
            ret = fts_sec_ex_mode_switch(MODE_COVER, DISABLE);
            if (ret >= 0) {
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!ts_data->charger_mode) {
            ...
            ret = fts_sec_ex_mode_switch(MODE_CHARGER, ENABLE);
            if (ret >= 0) {
        ...
        if (ts_data->charger_mode) {
            ...
            ret = fts_sec_ex_mode_switch(MODE_CHARGER, DISABLE);
            if (ret >= 0) {
...
static struct attribute *fts_sec_touch_mode_attrs[] = {
...
static struct attribute_group fts_sec_touch_mode_group = {
    .attrs = fts_sec_touch_mode_attrs,
};

int fts_sec_ex_mode_recovery(struct fts_ts_data *ts_data)
{
    if (ts_data->glove_mode) {
        fts_sec_ex_mode_switch(MODE_GLOVE, ENABLE);
    }

    if (ts_data->cover_mode) {
        fts_sec_ex_mode_switch(MODE_COVER, ENABLE);
    }

    if (ts_data->charger_mode) {
        fts_sec_ex_mode_switch(MODE_CHARGER, ENABLE);
    }
...
int fts_sec_ex_mode_init(struct fts_ts_data *ts_data)
{
    ...
    ret = sysfs_create_group(&ts_data->dev->kobj, &fts_sec_touch_mode_group);
    if (ret < 0) {
        FTS_ERROR("create sysfs(ex_mode) fail");
        sysfs_remove_group(&ts_data->dev->kobj, &fts_sec_touch_mode_group);
        return ret;
    } else {
