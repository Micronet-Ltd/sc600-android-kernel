...
struct upgrade_module module_list_sec[] = {
    {FTS_MODULE_ID, FTS_MODULE_NAME, fw_file_sec, sizeof(fw_file_sec)},
    {FTS_MODULE2_ID, FTS_MODULE2_NAME, fw_file2_sec, sizeof(fw_file2_sec)},
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, fw_file3_sec, sizeof(fw_file3_sec)},
};

struct upgrade_func *upgrade_func_list_sec[] = {
    &upgrade_func_ft5422_sec,
};

struct fts_upgrade *fwupgrade_sec;

/*****************************************************************************
* Static function prototypes
*****************************************************************************/
static bool fts_sec_fwupg_check_state(
...
***********************************************************************/
static int fts_sec_fwupg_get_boot_state(
    ...
    enum FW_STATUS *fw_sts)
{
    ...
    if (fts_sec_data->ic_info.is_incell)
        cmd_len = FTS_CMD_READ_ID_LEN_INCELL;
    else
        cmd_len = FTS_CMD_READ_ID_LEN;
    ret = fts_sec_read(cmd, cmd_len, val, 2);
    if (ret < 0) {
...
static int fts_sec_fwupg_reset_to_boot(struct fts_upgrade *upg)
{
    int ret = 0;
    ...
    ret = fts_sec_write_reg(reg, FTS_UPGRADE_AA);
    if (ret < 0) {
    ...
    ret = fts_sec_write_reg(reg, FTS_UPGRADE_55);
    if (ret < 0) {
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
static int fts_sec_fwupg_reset_to_romboot(struct fts_upgrade *upg)
{
    int ret = 0;
    int i = 0;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ret = fts_sec_fwupg_get_boot_state(upg, &state);
...
u16 fts_sec_crc16_calc_host(u8 *pbuf, u16 length)
{
    u16 ecc = 0;
    u16 i = 0;
    u16 j = 0;
...
static u16 fts_sec_pram_ecc_calc_host(u8 *pbuf, u16 length)
{
    return fts_sec_crc16_calc_host(pbuf, length);
}

static int fts_sec_pram_ecc_cal_algo(
    struct fts_upgrade *upg,
    u32 start_addr,
    u32 ecc_length)
{
    ...
    ret = fts_sec_write(cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
...
static int fts_sec_pram_ecc_cal_xor(void)
{
    int ret = 0;
    u8 reg_val = 0;
    ...
    ret = fts_sec_read_reg(FTS_ROMBOOT_CMD_ECC, &reg_val);
    if (ret < 0) {
        FTS_ERROR("read pramboot ecc fail");
        return ret;
    }
...
static int fts_sec_pram_ecc_cal(struct fts_upgrade *upg, u32 saddr, u32 len)
{
    if ((!upg) || (!upg->func)) {
        FTS_ERROR("upg/func is null");
        return -EINVAL;
    ...
    if (ECC_CHECK_MODE_CRC16 == upg->func->pram_ecc_check_mode) {
        return fts_sec_pram_ecc_cal_algo(upg, saddr, len);
    } else {
        return fts_sec_pram_ecc_cal_xor();
    }
}

static int fts_sec_pram_write_buf(struct fts_upgrade *upg, u8 *buf, u32 len)
{
    int ret = 0;
    u32 i = 0;
    u32 j = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_sec_write(packet_buf, packet_len + FTS_CMD_WRITE_LEN);
        if (ret < 0) {
    ...
    if (ECC_CHECK_MODE_CRC16 == upg->func->pram_ecc_check_mode) {
        ecc_in_host = (int)fts_sec_pram_ecc_calc_host(buf, len);
    } else {
...
static int fts_sec_pram_start(void)
{
...
static int fts_sec_pram_write_remap(struct fts_upgrade *upg)
{
    int ret = 0;
    int ecc_in_host = 0;
    int ecc_in_tp = 0;
    ...
    ecc_in_host = fts_sec_pram_write_buf(upg, pb_buf, pb_len);
    if (ecc_in_host < 0) {
    ...
    /* read out checksum */
    ecc_in_tp = fts_sec_pram_ecc_cal(upg, 0, pb_len);
    if (ecc_in_tp < 0) {
        FTS_ERROR( "read pramboot ecc fail");
        return ecc_in_tp;
    }
...
static int fts_sec_pram_init(void)
{
    int ret = 0;
    u8 reg_val = 0;
    u8 wbuf[3] = { 0 };
    ...
    ret = fts_sec_read(wbuf, 1, &reg_val, 1);
    if (ret < 0) {
...
static int fts_sec_pram_write_init(struct fts_upgrade *upg)
{
    int ret = 0;
    bool state = 0;
    ...
    if (status != FTS_RUN_IN_ROM) {
        ...
        ret = fts_sec_fwupg_reset_to_romboot(upg);
        if (ret < 0) {
            FTS_ERROR("reset to romboot fail");
            return ret;
        }
    ...
    ret = fts_sec_pram_write_remap(upg);
    if (ret < 0) {
    ...
    state = fts_sec_fwupg_check_state(upg, FTS_RUN_IN_PRAM);
    if (!state) {
...
static bool fts_sec_fwupg_check_fw_valid(void)
{
    int ret = 0;

    ret = fts_sec_wait_tp_to_valid();
    if (ret < 0) {
...
/************************************************************************
* Name: fts_sec_fwupg_check_state
    ...
    struct fts_upgrade *upg, enum FW_STATUS rstate)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ret = fts_sec_fwupg_get_boot_state(upg, &cstate);
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_sec_fwupg_reset_in_boot(void)
{
    int ret = 0;
    u8 cmd = FTS_CMD_RESET;
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_sec_fwupg_enter_into_boot(void)
{
    int ret = 0;
    bool fwvalid = false;
    bool state = false;
    struct fts_upgrade *upg = fwupgrade_sec;
    ...
    fwvalid = fts_sec_fwupg_check_fw_valid();
    if (fwvalid) {
        ret = fts_sec_fwupg_reset_to_boot(upg);
        if (ret < 0) {
            FTS_ERROR("enter into romboot/bootloader fail");
            return ret;
        }
    } else if (upg->func->read_boot_id_need_reset) {
        ret = fts_sec_fwupg_reset_in_boot();
        if (ret < 0) {
    ...
    if (upg->func->pramboot_supported) {
        ...
        /* pramboot */
        ret = fts_sec_pram_write_init(upg);
        if (ret < 0) {
        ...
        /* bootloader */
        state = fts_sec_fwupg_check_state(upg, FTS_RUN_IN_BOOTLOADER);
        if (!state) {
...
***********************************************************************/
static bool fts_sec_fwupg_check_flash_status(
    u16 flash_status,
    int retries,
    int retries_delay)
{
    ...
    for (i = 0; i < retries; i++) {
        ...
        ret = fts_sec_read(&cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_sec_fwupg_erase(u32 delay)
{
    int ret = 0;
    u8 cmd = 0;
    bool flag = false;
    ...
    flag = fts_sec_fwupg_check_flash_status(FTS_CMD_FLASH_STATUS_ERASE_OK,
                                        FTS_RETRIES_REASE,
 ...
 ***********************************************************************/
int fts_sec_fwupg_ecc_cal(u32 saddr, u32 len)
{
    int ret = 0;
    u32 i = 0;
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_sec_write(wbuf, FTS_CMD_ECC_CAL_LEN);
        if (ret < 0) {
        ...
        ret = fts_sec_fwupg_check_flash_status(FTS_CMD_FLASH_STATUS_ECC_OK,
                                           FTS_RETRIES_ECC_CAL,
    ...
    ret = fts_sec_read(wbuf, 1, val, ecc_len);
    if (ret < 0) {
    ...
    u32 delay)
{
    ...
    ecc_in_host = (int)ecc_tmp;
    if (ECC_CHECK_MODE_CRC16 == upg->func->fw_ecc_check_mode) {
        ecc_in_host = (int)fts_sec_crc16_calc_host(buf, len);
    }
 ...
 ***********************************************************************/
int fts_sec_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
    int ret = 0;
    u32 i = 0;
    u32 packet_number = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_sec_write(wbuf, FTS_CMD_READ_LEN);
        if (ret < 0) {
        ...
        ret = fts_sec_read(NULL, 0, buf + offset, packet_len);
        if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_sec_flash_read(u32 addr, u8 *buf, u32 len)
{
    int ret = 0;
    ...
    ret = fts_sec_fwupg_enter_into_boot();
    if (ret < 0) {
        FTS_ERROR("enter into pramboot/bootloader fail");
        goto read_flash_err;
    }

    ret = fts_sec_flash_read_buf(addr, buf, len);
    if (ret < 0) {
...
int fts_sec_read_file(char *file_name, u8 **file_buf)
{
    int ret = 0;
...
int fts_sec_upgrade_bin(char *fw_name, bool force)
{
    int ret = 0;
    u32 fw_file_len = 0;
    u8 *fw_file_buf = NULL;
    ...
    fts_sec_irq_disable();
#if FTS_ESDCHECK_EN
    fts_sec_esdcheck_switch(DISABLE);
#endif

    ret = fts_sec_read_file(fw_name, &fw_file_buf);
...
err_bin:
#if FTS_ESDCHECK_EN
    fts_sec_esdcheck_switch(ENABLE);
#endif
    fts_sec_irq_enable();
...
int fts_sec_enter_test_environment(bool test_state)
{
    return 0;
}
#if FTS_AUTO_LIC_UPGRADE_EN
static int fts_sec_lic_get_vid_in_tp(u16 *vid)
{
    int ret = 0;
    u8 val[2] = { 0 };
    ...
    ret = fts_sec_read_reg(FTS_REG_VENDOR_ID, &val[0]);
    if (fts_sec_data->ic_info.is_incell)
        ret = fts_sec_read_reg(FTS_REG_MODULE_ID, &val[1]);
    if (ret < 0) {
...
static int fts_sec_lic_get_vid_in_host(struct fts_upgrade *upg, u16 *vid)
{
    u8 val[2] = { 0 };
    u8 *licbuf = NULL;
    u32 conf_saddr = 0;
...
static int fts_sec_lic_get_ver_in_tp(u8 *ver)
{
    int ret = 0;
    ...
    ret = fts_sec_read_reg(FTS_REG_LIC_VER, ver);
    if (ret < 0) {
...
static int fts_sec_lic_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
    int ret = 0;
...
static bool fts_sec_lic_need_upgrade(struct fts_upgrade *upg)
{
    int ret = 0;
    ...
    ret = fts_sec_lic_get_vid_in_host(upg, &vid_in_host);
    if (ret < 0) {
        FTS_ERROR("vendor id in host invalid");
        return false;
    }

    ret = fts_sec_lic_get_vid_in_tp(&vid_in_tp);
    if (ret < 0) {
    ...
    ret = fts_sec_lic_get_ver_in_host(upg, &initcode_ver_in_host);
    if (ret < 0) {
        FTS_ERROR("init code in host invalid");
        return false;
    }

    ret = fts_sec_lic_get_ver_in_tp(&initcode_ver_in_tp);
    if (ret < 0) {
...
static int fts_sec_lic_upgrade(struct fts_upgrade *upg)
{
    int ret = 0;
    ...
    hlic_upgrade = fts_sec_lic_need_upgrade(upg);
...
static int fts_sec_param_get_ver_in_tp(u8 *ver)
{
    int ret = 0;
    ...
    ret = fts_sec_read_reg(FTS_REG_IDE_PARA_VER_ID, ver);
    if (ret < 0) {
...
static int fts_sec_param_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
    if ((!upg) || (!upg->func) || (!upg->fw) || (!ver)) {
        FTS_ERROR("fts_sec_data/upgrade/func/fw/ver is NULL");
        return -EINVAL;
    }
 ...
 *         == 1: ide
 */
static int fts_sec_param_ide_in_host(struct fts_upgrade *upg)
{
    u32 off = 0;

    if ((!upg) || (!upg->func) || (!upg->fw)) {
        FTS_ERROR("fts_sec_data/upgrade/func/fw is NULL");
        return -EINVAL;
    }
 ...
 *         1   : ide
 */
static int fts_sec_param_ide_in_tp(u8 *val)
{
    int ret = 0;

    ret = fts_sec_read_reg(FTS_REG_IDE_PARA_STATUS, val);
    if (ret < 0) {
...
/************************************************************************
 * fts_sec_param_need_upgrade - check fw paramcfg need upgrade or not
 *
 ...
 ***********************************************************************/
static int fts_sec_param_need_upgrade(struct fts_upgrade *upg)
{
    int ret = 0;
    u8 val = 0;
    int ide_in_host = 0;
    ...
    ide_in_host = fts_sec_param_ide_in_host(upg);
    if (ide_in_host < 0) {
        FTS_INFO("fts_param_ide_in_host fail");
        return ide_in_host;
    }

    ide_in_tp = fts_sec_param_ide_in_tp(&val);
    if (ide_in_tp < 0) {
    ...
    if ((0 == ide_in_host) && (0 == ide_in_tp)) {
        ...
        ret = fts_sec_param_get_ver_in_host(upg, &ver_in_host);
        if (ret < 0) {
            FTS_ERROR("param version in host invalid");
            return ret;
        }

        ret = fts_sec_param_get_ver_in_tp(&ver_in_tp);
        if (ret < 0) {
...
static int fts_sec_fwupg_get_ver_in_tp(u8 *ver)
{
    int ret = 0;
    ...
    ret = fts_sec_read_reg(FTS_REG_FW_VER, ver);
    if (ret < 0) {
...
static int fts_sec_fwupg_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
    if ((!upg) || (!upg->func) || (!upg->fw) || (!ver)) {
        FTS_ERROR("fts_sec_data/upgrade/func/fw/ver is NULL");
        return -EINVAL;
...
static bool fts_sec_fwupg_need_upgrade(struct fts_upgrade *upg)
{
    int ret = 0;
    bool fwvalid = false;
    u8 fw_ver_in_host = 0;
    ...
    fwvalid = fts_sec_fwupg_check_fw_valid();
    if (fwvalid) {
        ret = fts_sec_fwupg_get_ver_in_host(upg, &fw_ver_in_host);
        if (ret < 0) {
            FTS_ERROR("get fw ver in host fail");
            return false;
        }

        ret = fts_sec_fwupg_get_ver_in_tp(&fw_ver_in_tp);
        if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_sec_fwupg_upgrade(struct fts_upgrade *upg)
{
    int ret = 0;
    ...
    upgrade_flag = fts_sec_fwupg_need_upgrade(upg);
        ...
        if (upgrade_flag) {
            ...
            if (upg->func->upgrade) {
                ...
                if (ret < 0) {
                    ...
                    fts_sec_fwupg_get_ver_in_tp(&ver);
            ...
            if (upg->func->param_upgrade) {
                ret = fts_sec_param_need_upgrade(upg);
                if (ret <= 0) {
                    ...
                    if (ret < 0) {
                        ...
                        fts_sec_param_get_ver_in_tp(&ver);
...
/************************************************************************
 * fts_sec_fwupg_auto_upgrade - upgrade main entry
 ***********************************************************************/
static void fts_sec_fwupg_auto_upgrade(struct fts_upgrade *upg)
{
    int ret = 0;
...
static int fts_sec_fwupg_get_vendorid(struct fts_upgrade *upg, int *vid)
{
    int ret = 0;
    bool fwvalid = false;
    u8 vendor_id = 0;
    ...
    fwvalid = fts_sec_fwupg_check_fw_valid();
    if (fwvalid) {
        ret = fts_sec_read_reg(FTS_REG_VENDOR_ID, &vendor_id);
        if (upg->ts_data->ic_info.is_incell)
            ret = fts_sec_read_reg(FTS_REG_MODULE_ID, &module_id);
    } else {
        fwcfg_addr =  upg->func->fwcfgoff;
        ret = fts_sec_flash_read(fwcfg_addr, cfgbuf, FTS_HEADER_LEN);
...
static int fts_sec_fwupg_get_module_info(struct fts_upgrade *upg)
{
    int ret = 0;
    int i = 0;
    struct upgrade_module *info = &module_list_sec[0];
    ...
    if (FTS_GET_MODULE_NUM > 1) {
        ...
        ret = fts_sec_fwupg_get_vendorid(upg, &upg->module_id);
        if (ret < 0) {
...
static int fts_sec_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    int ret = 0;
...
static int fts_sec_get_fw_file_via_i(struct fts_upgrade *upg)
{
 ...
 *  Return: return 0 if success, otherwise return error code
 *****************************************************************************/
static int fts_sec_fwupg_get_fw_file(struct fts_upgrade *upg)
{
    int ret = 0;
    ...
    ret = fts_sec_fwupg_get_module_info(upg);
    ...
    if (FTS_FW_REQUEST_SUPPORT) {
        ret = fts_sec_get_fw_file_via_request_firmware(upg);
        if (ret != 0) {
            get_fw_i_flag = true;
        }
    } else {
    ...
    if (get_fw_i_flag) {
        ret = fts_sec_get_fw_file_via_i(upg);
    }
...
static void fts_sec_fwupg_init_ic_detail(struct fts_upgrade *upg)
{
...
/*****************************************************************************
 *  Name: fts_sec_fwupg_work
 *  Brief: 1. get fw image/file
 *         2. ic init if have
 *         3. call upgrade main function(fts_sec_fwupg_auto_upgrade)
 *  Input:
 *  Output:
 *  Return:
 *****************************************************************************/
static void fts_sec_fwupg_work(struct work_struct *work)
{
    int ret = 0;
    struct fts_upgrade *upg = fwupgrade_sec;
    ...
    /* get fw */
    ret = fts_sec_fwupg_get_fw_file(upg);
    if (ret < 0) {
        FTS_ERROR("get file fail, can't upgrade");
    } else {
        /* ic init if have */
        fts_sec_fwupg_init_ic_detail(upg);
        /* run auto upgrade */
        fts_sec_fwupg_auto_upgrade(upg);
    }
...
int fts_sec_fwupg_init(struct fts_ts_data *ts_data)
{
    int i = 0;
    int j = 0;
    int ic_stype = 0;
    struct upgrade_func *func = upgrade_func_list_sec[0];
    int func_count = sizeof(upgrade_func_list_sec) / sizeof(upgrade_func_list_sec[0]);
    ...
    fwupgrade_sec = (struct fts_upgrade *)kzalloc(sizeof(*fwupgrade_sec), GFP_KERNEL);
    if (NULL == fwupgrade_sec) {
    ...
    if (1 == func_count) {
        fwupgrade_sec->func = func;
    } else {
        for (i = 0; i < func_count; i++) {
            func = upgrade_func_list_sec[i];
    ...
    if (NULL == fwupgrade_sec->func) {
        FTS_ERROR("no upgrade function match, can't upgrade");
        return -ENODATA;
    }

    fwupgrade_sec->ts_data = ts_data;
    INIT_WORK(&ts_data->fwupg_work, fts_sec_fwupg_work);
    queue_work(ts_data->ts_workqueue, &ts_data->fwupg_work);

    return 0;
}

int fts_sec_fwupg_exit(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    if (fwupgrade_sec) {
        if (fwupgrade_sec->fw_from_request) {
            vfree(fwupgrade_sec->fw);
            fwupgrade_sec->fw = NULL;
        }
