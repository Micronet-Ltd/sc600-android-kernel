...
int fts_sec_create_gesture_sysfs(struct device *dev)
{
    int ret = 0;
...
static void fts_sec_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    int gesture;
...
int fts_sec_gesture_readdata(struct fts_ts_data *ts_data, u8 *data)
{
    ...
    ret = fts_sec_read_reg(FTS_REG_GESTURE_EN, &buf[0]);
    ...
    ret = fts_sec_read(&buf[2], 1, &buf[2], FTS_GESTURE_DATA_LEN - 2);
    if (ret < 0) {
    ...
    fts_sec_gesture_report(input_dev, gesture->gesture_id);
    return 0;
}
...
void fts_sec_gesture_recovery(struct fts_ts_data *ts_data)
{
    if ((ENABLE == fts_gesture_data.mode) && (ENABLE == fts_gesture_data.active)) {
        ...
        fts_sec_write_reg(0xD1, 0xFF);
        fts_sec_write_reg(0xD2, 0xFF);
        fts_sec_write_reg(0xD5, 0xFF);
        fts_sec_write_reg(0xD6, 0xFF);
        fts_sec_write_reg(0xD7, 0xFF);
        fts_sec_write_reg(0xD8, 0xFF);
        fts_sec_write_reg(FTS_REG_GESTURE_EN, ENABLE);
    }
}
...
int fts_sec_gesture_suspend(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        fts_sec_write_reg(FTS_REG_GESTURE_EN, ENABLE);
        msleep(1);
        fts_sec_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
    }
...
int fts_sec_gesture_resume(struct fts_ts_data *ts_data)
{
    ...
    fts_gesture_data.active = DISABLE;
    for (i = 0; i < 5; i++) {
        fts_sec_write_reg(FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        fts_sec_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
...
int fts_sec_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    fts_sec_create_gesture_sysfs(ts_data->dev);
