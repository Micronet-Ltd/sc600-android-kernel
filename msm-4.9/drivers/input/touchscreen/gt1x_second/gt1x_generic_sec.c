...
struct gt1x_version_info gt1x_sec_version = {
	.product_id = {0},
	.patch_id = 0,
	.mask_id = 0,
	.sensor_id = 0,
...
static struct proc_dir_entry *gt1x_second_debug_proc_entry = NULL;
static const struct file_operations gt1x_second_debug_fops = {
	.owner = THIS_MODULE,
...
{
	gt1x_second_debug_proc_entry = proc_create(GT1X_DEBUG_PROC_FILE, 0660, NULL, &gt1x_second_debug_fops);
	if (gt1x_second_debug_proc_entry == NULL) {
...
/**
 * gt1x_sec_read_version - Read gt1x version info.
 * @ver_info: address to store version info
 * Return 0-succeed.
 */
s32 gt1x_sec_read_version(struct gt1x_version_info * ver_info)
{
	s32 ret = -1;
	u8 buf[12] = { 0 };
	u32 mask_id = 0;
...
s32 gt1x_sec_get_chip_type(void)
{
	...
	if (!memcmp(opr_buf, gt1x_data, sizeof(gt1x_data))) {
		gt1x_sec_chip_type = CHIP_TYPE_GT1X;
	} else if (!memcmp(opr_buf, gt9l_data, sizeof(gt9l_data))) {
		gt1x_sec_chip_type = CHIP_TYPE_GT2X;
	}

	if (gt1x_sec_chip_type != CHIP_TYPE_NONE) {
		GTP_INFO("Chip Type: %s", (gt1x_sec_chip_type == CHIP_TYPE_GT1X) ? "GT1X" : "GT2X");
		return 0;
	} else {
		return -1;
	}
...
static s32 gt1x_ps_enable(s32 enable)
{
	...
	state = enable ? 1 : 0;
    if (gt1x_sec_chip_type == CHIP_TYPE_GT1X)
	    ret = gt1x_sec_i2c_write(GTP_REG_PROXIMITY_ENABLE, &state, 1);
    else if (gt1x_sec_chip_type == CHIP_TYPE_GT2X)
...
static int gt1x_ps_init(void)
{
	...
	gt1x_ps_dev->input_dev->id.bustype = BUS_USB;
...
static void gt1x_init_charger(void)
{
    ...
    if (gt1x_parse_chr_cfg(gt1x_sec_version.sensor_id) < 0) {
...
void gt1x_charger_config(s32 dir_update)
{
	...
	if (gt1x_get_charger_status()) {
		if (!chr_pluggedin || dir_update) {
			...
			if (gt1x_sec_send_cfg(gt1x_config_charger, gt1x_sec_cfg_length)) {
