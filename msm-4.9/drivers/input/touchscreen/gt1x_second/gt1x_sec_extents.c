/* drivers/input/touchscreen/gt1x_sec_extents.c
 *
...
#if 0
static ssize_t gt1x_sec_gesture_data_read(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	s32 ret = -1;
	GTP_DEBUG("visit gt1x_sec_gesture_data_read. ppos:%d", (int)*ppos);
	if (*ppos) {
		return 0;
	}
	if (size == 4) {
		ret = copy_to_user(((u8 __user *) page), "GT1X_SEC", 4);
		return 4;
	}
...
static ssize_t gt1x_sec_gesture_data_write(struct file *filp, const char __user * buff, size_t len, loff_t * off)
{
	s32 ret = 0;
...
static ssize_t gt1x_sec_gesture_data_read(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	...
	*/
	...
	ret = copy_to_user(page, &enable, 1);
	GTP_DEBUG("Got the gsture_enabled=%d\n", gesture_enabled);
	return ret;
}
...
s32 gesture_event_handler(struct input_dev * dev)
{
    ...
    /** package: -head 4B + track points + extra info- 
        ...
        */
	ret = gt1x_sec_i2c_read(GTP_REG_WAKEUP_GESTURE, doze_buf, 4);
	if (ret < 0) {
        return 0;
    }
    ...
    if (extra_len >= 0) {
    	...
    	ret = gt1x_sec_i2c_read(GTP_REG_WAKEUP_GESTURE + 4,
	...
	if (ges_type == 0 || !QUERYBIT(gestures_flag, ges_type)) {
        ...
        doze_buf[0] = 0x00;
		gt1x_sec_i2c_write(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);
        gesture_enter_doze();
		return 0;
	}
    ...
    if (len > 0) { /* coor num * 4 + chksum 2*/
        ...
        ret = gt1x_sec_i2c_read(GES_BUFFER_ADDR, ges_data, len * 4);
        if (ret < 0) {
        ...
        ret = gt1x_sec_i2c_read(0x819F, &ges_data[len * 4], 2);
        if (ret < 0) {
            GTP_ERROR("Read gesture data failed.");
            return 0;
        }
...
void gt1x_sec_gesture_debug(int on) 
{
    if (on) {
        gesture_enabled = 1;
        memset(gestures_flag, 0xFF, sizeof(gestures_flag));
...
#define HOTKNOT_VERSION  "GOODIX,GT1X_SEC"
...
static s32 hotknot_enter_transfer_mode(void)
{
	...
	gt1x_sec_send_cmd(GTP_CMD_HN_TRANSFER, 0);
	msleep(100);
	gt1x_sec_irq_enable();

	ret = gt1x_sec_i2c_read(0x8140, buffer, sizeof(buffer));
	if (ret) {
...
static s32 hotknot_load_authentication_subsystem(void)
{
	s32 ret = 0;
	u8 buffer[5] = { 0 };
	ret = gt1x_sec_hold_ss51_dsp_no_reset();
	if (ret < 0) {
	...
	if (gt1x_sec_chip_type == CHIP_TYPE_GT1X_SEC) {
		...
		ret = gt1x_sec_load_patch(gt1x_sec_patch_jump_fw, 4096, 0, 1024 * 8);
		if (ret < 0) {
			GTP_ERROR("Load jump code fail!");
			return ret;
		}
		GTP_INFO("hotknot load auth code.");
		ret = gt1x_sec_load_patch(hotknot_auth_fw, 4096, 4096, 1024 * 8);
		if (ret < 0) {
		...
		GTP_INFO("hotknot load auth code.");
		ret = gt1x_sec_load_patch(hotknot_auth_fw, 4096, 0, 1024 * 6);
		if (ret < 0) {
			GTP_ERROR("load auth system fail!");
			return ret;
		}
	...
	ret = gt1x_sec_startup_patch();
	if (ret < 0) {
		GTP_ERROR("Startup auth system fail!");
		return ret;
	}
	ret = gt1x_sec_i2c_read(GTP_REG_VERSION, buffer, 4);
	if (ret < 0) {
...
s32 hotknot_event_handler(u8 * data)
{
	...
	if (hotknot_paired_flag) {
		s32 ret = -1;
		ret = gt1x_sec_i2c_read(GTP_REG_HN_STATE, hn_state_buf, 6);
		if (ret < 0) {
...
static s32 io_iic_read(u8 * data, void __user * arg)
{
	...
	err = gt1x_sec_i2c_read(addr, &data[CMD_HEAD_LENGTH], data_length);
	if (!err) {
...
static s32 io_iic_write(u8 * data)
{
	...
	err = gt1x_sec_i2c_write(addr, &data[CMD_HEAD_LENGTH], data_length);
	if (!err) {
		err = CMD_HEAD_LENGTH + data_length;
	}
...
//         error otherwise.
static long gt1x_sec_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	u32 value = 0;
...
#ifdef CONFIG_COMPAT
static long gt1x_sec_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static const struct file_operations gt1x_sec_fops = {
	.owner = THIS_MODULE,
#if GTP_GESTURE_WAKEUP
	.read = gt1x_sec_gesture_data_read,
	.write = gt1x_sec_gesture_data_write,
#endif
	.unlocked_ioctl = gt1x_sec_ioctl,
#ifdef CONFIG_COMPAT
    .compat_ioctl = gt1x_sec_compat_ioctl,
#endif
};
...
s32 gt1x_sec_init_node(void)
{
	...
	proc_entry = proc_create(GESTURE_NODE, 0666, NULL, &gt1x_sec_fops);
	if (proc_entry == NULL) {
...
void gt1x_sec_deinit_node(void)
{
#if GTP_GESTURE_WAKEUP
