/* drivers/input/touchscreen/gt1x_sec_generic.h
 *
...
#ifndef _GT1X_SEC_GENERIC_H_
#define _GT1X_SEC_GENERIC_H_
...
#define GTP_HEADER_FW_UPDATE  0	// firmware in gt1x_sec_firmware.h
...
#define GTP_I2C_NAME                "Goodix_sec-TP"
#define GT1X_SEC_DEBUG_PROC_FILE        "gt1x_sec_debug"
...
#define GTP_BAK_REF_PATH                "/data/gt1x_sec_ref.bin"
#define GTP_MAIN_CLK_PATH               "/data/gt1x_sec_clk.bin"
...
//Log define
#define GTP_INFO(fmt,arg...)           printk("<<GTP_sec-INF>>[%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define GTP_ERROR(fmt,arg...)          printk("<<GTP_sec-ERR>>[%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define GTP_DEBUG(fmt,arg...)          do{\
                                         if(GTP_DEBUG_ON)\
                                         printk("<<GTP_sec-DBG>>[%s:%d]"fmt"\n",__func__, __LINE__, ##arg);\
                                       }while(0)
...
typedef enum {
	CHIP_TYPE_GT1X_SEC = 0,
	CHIP_TYPE_GT2X_SEC = 1,
	CHIP_TYPE_NONE = 0xFF
} CHIP_TYPE_T;
...
/* Export from gt1x_sec_extents.c and gt1x_sec_firmware.h */
#if GTP_HOTKNOT
extern u8 hotknot_enabled;
extern u8 hotknot_transfer_mode;
extern u8 gt1x_sec_patch_jump_fw[];
...
extern s32 gt1x_sec_init_node(void);
extern void gt1x_sec_deinit_node(void);

#if GTP_GESTURE_WAKEUP
extern DOZE_T gesture_doze_status;
extern struct mutex gesture_sec_doze_lock; /*kyle: add lock for gesture_doze_status */
extern int gesture_enabled;
extern void gt1x_sec_gesture_debug(int on) ;
...
/* Export from gt1x_sec_tpd.c */
extern void gt1x_sec_touch_down(s32 x, s32 y, s32 size, s32 id);
extern void gt1x_sec_touch_up(s32 id);
extern int gt1x_sec_power_switch(s32 state);
extern void gt1x_sec_irq_enable(void);
extern void gt1x_sec_irq_disable(void);
extern int gt1x_sec_debug_proc(u8 * buf, int count);
...
/* Export form gt1x_sec_update.c */
extern struct fw_update_info update_info;

extern u8 gt1x_sec_default_FW[];
extern int gt1x_sec_hold_ss51_dsp(void);
extern int gt1x_sec_auto_update_proc(void *data);
extern int gt1x_sec_update_firmware(void *filename);
extern void gt1x_sec_enter_update_mode(void);
extern void gt1x_sec_leave_update_mode(void);
extern int gt1x_sec_hold_ss51_dsp_no_reset(void);
extern int gt1x_sec_load_patch(u8 * patch, u32 patch_size, int offset, int bank_size);
extern int gt1x_sec_startup_patch(void);

/* Export from gt1x_sec_tool.c */
#if GTP_CREATE_WR_NODE
extern int gt1x_sec_init_tool_node(void);
extern void gt1x_sec_deinit_tool_node(void);
#endif

/* Export from gt1x_sec_generic.c */
extern struct i2c_client *gt1x_sec_i2c_client;

extern CHIP_TYPE_T gt1x_sec_chip_type;
extern struct gt1x_sec_version_info gt1x_sec_version;

extern s32 _do_sec_i2c_read(struct i2c_msg *msgs, u16 addr, u8 * buffer, s32 len);
extern s32 _do_sec_i2c_write(struct i2c_msg *msg, u16 addr, u8 * buffer, s32 len);
extern s32 gt1x_sec_i2c_write(u16 addr, u8 * buffer, s32 len);
extern s32 gt1x_sec_i2c_read(u16 addr, u8 * buffer, s32 len);
extern s32 gt1x_sec_i2c_read_dbl_check(u16 addr, u8 * buffer, s32 len);

extern u8 gt1x_sec_config[];
extern u32 gt1x_sec_cfg_length;
extern u8 gt1x_sec_int_type;
extern u8 gt1x_sec_wakeup_level;
extern u32 gt1x_sec_abs_x_max;
extern u32 gt1x_sec_abs_y_max;
extern u8 gt1x_sec_init_failed;
extern int gt1x_sec_halt;
extern volatile int gt1x_sec_rawdiff_mode;

extern s32 gt1x_sec_init(void);
extern void gt1x_sec_deinit(void);
extern s32 gt1x_sec_read_version(struct gt1x_sec_version_info *ver_info);
extern s32 gt1x_sec_init_panel(void);
extern s32 gt1x_sec_get_chip_type(void);
extern s32 gt1x_sec_request_event_handler(void);
extern int gt1x_sec_send_cmd(u8 cmd, u8 data);
extern s32 gt1x_sec_send_cfg(u8 * config, int cfg_len);
extern void gt1x_sec_select_addr(void);
extern s32 gt1x_sec_reset_guitar(void);
extern void gt1x_sec_power_reset(void);
extern int gt1x_sec_parse_config(char *filename, u8 * gt1x_sec_config);
extern s32 gt1x_sec_touch_event_handler(u8 * data, struct input_dev *dev, struct input_dev *pen_dev);
extern int gt1x_sec_suspend(void);
extern int gt1x_sec_resume(void);

#if GTP_HAVE_TOUCH_KEY
extern const u16 gt1x_sec_touch_key_array[];
#endif

#if GTP_WITH_STYLUS
extern struct input_dev *pen_dev;
extern void gt1x_sec_pen_up(s32 id);
extern void gt1x_sec_pen_down(s32 x, s32 y, s32 size, s32 id);
#endif

#if GTP_PROXIMITY
extern u8 gt1x_sec_proximity_flag;
extern int gt1x_sec_prox_event_handler(u8 * data);
#endif

#if GTP_SMART_COVER
extern int gt1x_sec_parse_sc_cfg(int sensor_id);
#endif

#if GTP_ESD_PROTECT
extern void gt1x_sec_init_esd_protect(void);
extern void gt1x_sec_esd_switch(s32 on);
#endif

#if GTP_CHARGER_SWITCH
extern u32 gt1x_sec_get_charger_status(void);
extern void gt1x_sec_charger_switch(s32 on);
extern void gt1x_sec_charger_config(s32 dir_update);
extern int gt1x_sec_parse_chr_cfg(int sensor_id);
#endif

#endif // _GT1X_SEC_GENERIC_H_
