 ...
 *      V1.2:
 *          modify gtp_sec_reset_guitar,slot report,tracking_id & 0x0F.
...
#define GOODIX_DEV_NAME	"Goodix_sec-CTP"
...
bool gtp_sec_init_done;
static u8 chip_gt9xxs;  /* true if ic is gt9xxs, like gt915s */
u8 grp_sec_cfg_version;
struct i2c_client  *gtp_sec_i2c_connect_client;

#define GTP_DEBUGFS_DIR			"ts_sec_debug"
...
*********************************************************/
int gtp_sec_i2c_read(struct i2c_client *client, u8 *buf, int len)
{
...
//		dev_err(&client->dev, "I2C retry: %d\n", retries + 1);
	...
	if (retries == GTP_I2C_RETRY_5) {
		...
		if (gtp_sec_init_done)
			gtp_sec_reset_guitar(ts, 10);
		else
			dev_warn(&client->dev,
				"gtp_sec_reset_guitar exit gtp_sec_init_done=%d:\n",
				gtp_sec_init_done);
	}
	return ret;
}
...
*********************************************************/
int gtp_sec_i2c_write(struct i2c_client *client, u8 *buf, int len)
{
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
	int ret = -EIO;
	u8 retries;
...
*********************************************************/
int gtp_sec_i2c_read_dbl_check(struct i2c_client *client,
			u16 addr, u8 *rxbuf, int len)
{
	...
	while (retry++ < GTP_I2C_RETRY_3) {
		...
		buf[0] = (u8)(addr >> 8);
		buf[1] = (u8)(addr & 0xFF);
		gtp_sec_i2c_read(client, buf, len + 2);

		memset(confirm_buf, 0xAB, 16);
		confirm_buf[0] = (u8)(addr >> 8);
		confirm_buf[1] = (u8)(addr & 0xFF);
		gtp_sec_i2c_read(client, confirm_buf, len + 2);
...
*********************************************************/
int gtp_sec_send_cfg(struct goodix_ts_data *ts)
{
	int ret = 0;
	int retry;
	...
	if (ts->pdata->driver_send_cfg) {
		if (ts->fixed_cfg) {
			...
			for (retry = 0; retry < GTP_I2C_RETRY_5; retry++) {
				ret = gtp_sec_i2c_write(ts->client,
					ts->config_data,
...
Output:
	None.
*********************************************************/
void gtp_sec_irq_disable(struct goodix_ts_data *ts)
{
...
Output:
	None.
*********************************************************/
void gtp_sec_irq_enable(struct goodix_ts_data *ts)
{
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	if (ts->pdata->slide_wakeup) {
		if (DOZE_ENABLED == doze_status) {
			ret = gtp_sec_i2c_read(ts->client, doze_buf, 3);
			if (ret > 0) {
				if (doze_buf[2] == 0xAA) {
					dev_dbg(&ts->client->dev,
					...
					/* clear 0x814B */
					doze_buf[2] = 0x00;
					gtp_sec_i2c_write(ts->client, doze_buf, 3);
			...
			if (ts->use_irq)
				gtp_sec_irq_enable(ts);

			return;
		}
	}

	ret = gtp_sec_i2c_read(ts->client, point_data, 12);
	if (ret < 0) {
	...
	if (touch_num > 1) {
		...
		ret = gtp_sec_i2c_read(ts->client, buf,
				2 + 8 * (touch_num - 1));
...
exit_work_func:
	if (!ts->gtp_rawdiff_mode) {
		ret = gtp_sec_i2c_write(ts->client, end_cmd, 3);
		if (ret < 0)
...
Output:
	None.
*******************************************************/
void gtp_sec_reset_guitar(struct goodix_ts_data *ts, int ms)
{
...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
	...
	while (retry++ < GTP_I2C_RETRY_3) {
		...
		ret = gtp_sec_i2c_write(ts->client, i2c_control_buf, 3);
		if (ret < 0) {
			dev_err(&ts->client->dev,
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	if (ts->pdata->enable_power_off) {
		...
		gtp_sec_reset_guitar(ts, 20);

		ret = gtp_sec_send_cfg(ts);
		if (ret <= 0) {
			dev_err(&ts->client->dev,
...
static int gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	if (ts->pdata->driver_send_cfg) {
		for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++)
			dev_dbg(&client->dev, "Config Groups(%d) Lengths: %lu",
				i, ts->pdata->config_data_len[i]);

		ret = gtp_sec_i2c_read_dbl_check(ts->client, 0x41E4, opr_buf, 1);
		if (SUCCESS == ret) {
			if (opr_buf[0] != 0xBE) {
				ts->fw_error = 1;
				dev_err(&client->dev,
		...
		if (i == GOODIX_MAX_CFG_GROUP) {
			...
			ret = gtp_sec_i2c_read_dbl_check(ts->client,
		...
		ret = gtp_sec_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA,
			&opr_buf[0], 1);
		if (ret == SUCCESS) {
			if (opr_buf[0] < 90) {
		...
		ret = gtp_sec_i2c_read(ts->client, config_data,
...
static int gtp_read_fw_version(struct i2c_client *client, u16 *version)
{
	...
	ret = gtp_sec_i2c_read(client, buf, sizeof(buf));
	if (ret < 0) {
...
static int gtp_i2c_test(struct i2c_client *client)
{
	...
	while (retry--) {
		ret = gtp_sec_i2c_read(client, buf, 3);
		if (ret > 0)
			return ret;
//		dev_err(&client->dev, "GTP i2c test failed time %d.\n", retry);
	...
	if(retry < 0)
		dev_err(&client->dev, "GTP i2c test failed times %d.\n", GTP_I2C_RETRY_5);
	return ret;
}
...
static int gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	if (gpio_is_valid(pdata->power_gpio)) {
		...
		if (ret) {
			dev_err(&client->dev, "The second Tp unable to request power_en gpio [%d]\n",
				pdata->power_gpio);
			pdata->power_gpio = -1;
			//goto err_free_reset_gpio;
		}else{
			...
			if (ret) {
				dev_err(&client->dev, "The second Tp unable to set direction for power_en gpio [%d]\n", pdata->power_gpio);
				goto err_free_power_gpio;
			}
		}
	} else {
		dev_err(&client->dev, "The second Tp invalid power gpio [%d]!\n",
			pdata->power_gpio);
		ret = -EINVAL;
		goto err_free_reset_gpio;
	}
...
Output:
	...
	0: succeed, -1: failed.
*******************************************************/
static int gtp_sec_request_irq(struct goodix_ts_data *ts)
{
	int ret = 0;
...
static int goodix_power_init(struct goodix_ts_data *ts)
{
	...
	ts->avdd = regulator_get_optional(&ts->client->dev, "avdd");
	if (IS_ERR(ts->avdd)) {
		ret = PTR_ERR(ts->avdd);
		dev_info(&ts->client->dev,
	...
	ts->vdd = regulator_get_optional(&ts->client->dev, "vdd");
	if (IS_ERR(ts->vdd)) {
		ret = PTR_ERR(ts->vdd);
		dev_info(&ts->client->dev,
...
static int gtp_debug_data_get(void *_data, u64 *val)
{
	...
	if (gtp_debug_addr_is_valid(ts->addr)) {
		ret = gtp_sec_i2c_read(ts->client, buf, 3);
		if (ret < 0)
			dev_err(&ts->client->dev,
			   ...
			   const struct i2c_device_id *id)
{
	...
	gtp_sec_i2c_connect_client = client;
...
#if GTP_ESD_PROTECT
	gtp_sec_esd_switch(client, SWITCH_ON);
#endif
...
#if GTP_ESD_PROTECT
	gtp_sec_esd_switch(ts->client, SWITCH_OFF);
#endif
...
#if GTP_ESD_PROTECT
	gtp_sec_esd_switch(ts->client, SWITCH_ON);
#endif
	mutex_unlock(&ts->lock);
	ts->gtp_is_suspend = 0;
...
Output:
	void
*********************************************************/
void gtp_sec_esd_switch(struct i2c_client *client, int on)
{
	struct goodix_ts_data *ts;
...
static int gtp_init_ext_watchdog(struct i2c_client *client)
{
	/* in case of recursively reset by calling gtp_sec_i2c_write*/
	struct i2c_msg msg;
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	ts = i2c_get_clientdata(gtp_sec_i2c_connect_client);
	...
	for (retry = 0; retry < GTP_I2C_RETRY_3; retry++) {
		ret = gtp_sec_i2c_read(ts->client, test, 4);
		...
		if ((ret < 0)) {
			...
			if ((test[2] == 0xAA) || (test[3] != 0xAA)) {
				...
				test[2] = 0xAA;
				gtp_sec_i2c_write(ts->client, test, 3);
				break;
			}
		}
	}
